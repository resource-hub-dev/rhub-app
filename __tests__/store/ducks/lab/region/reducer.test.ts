...
import { TheRedYetiIcon } from '@patternfly/react-icons';
...
describe('region reducer', () => {
  ...
  test('handles load product regions request', () => {
    expect(reducer(INITIAL_STATE, actions.loadProductRegionsRequest(1))).toEqual({
      ...INITIAL_STATE,
      loading: true,
      error: false
    });
  ...
  test('handles create request', () => {
    ...
    expect(reducer(
      INITIAL_STATE,
      actions.createRequest(apiRequestBody)
    )).toEqual({
      ...INITIAL_STATE,
      loading: true,
      error: false
  ...
  test('handles update request', () => {
    ...
    expect(reducer(
      INITIAL_STATE,
      actions.updateRequest(1, apiRequestBody)
    )).toEqual({
      ...INITIAL_STATE,
      loading: true,
      error: false
